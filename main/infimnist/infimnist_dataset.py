...
class InfiMNIST(data.Dataset):
    def __init__(self, train=True, num_transformations=1, transform=None, target_transform=None):
        self.mnist = im.InfimnistGenerator()
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            img = torch.unsqueeze(self.transform(img), 0) if self.train else self.transform(img)

        target = torch.tensor([labels[0]] if self.train else labels[0], dtype=torch.int64)
