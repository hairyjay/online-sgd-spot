...
def add_plot(json_file):
    ...
    drift_end = [data["drift_start"][i] + data["drift_time"][i] for i in range(len(drift_start))]
    drift_time = data["drift_time"]
    for i in range(len(drift_start)):
        print(drift_start[i], drift_end[i], drift_time[i])
    ...
    for i in range(len(drift_start)):
        ax1.plot([drift_start[i], drift_end[i]], [12.5+i*7.5, 12.5+i*7.5], color=color, label="$N = {}$".format(data["size"]))

        # price = get_price_trace(run)
        ax1.fill_between([drift_start[i], drift_end[i]], [12.5+i*7.5, 12.5+i*7.5], [100, 100], color=color, label="$N = {}$".format(data["size"]), alpha=.1, linewidth=0.0)
        ax1.text(drift_start[i] - 100, 10+i*7.5, "{}s".format(drift_time[i]), color=color, horizontalalignment='right')
...
for run in os.scandir('../runs/drift_2026'):
